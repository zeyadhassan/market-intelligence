...
# FI-INTEL FORK CHANGE: the upstream commercial search tool and the
# model-provider native search helpers are DELETED (not disabled). Retrieval
# runs over the private licensed corpus only; no code path reaches the open
# internet for content (invariant 2). The upstream search/async-search, the
# two provider native-search detectors, and the search API-key helper are
...
async def get_all_tools(config: RunnableConfig):
    ...
    FI-INTEL FORK CHANGE: the open-web search tools are deleted. The toolkit
    is core research tools (ResearchComplete + reflection) plus optional MCP
    tools. Corpus retrieval is provided to the agent through
    fi_intel.tools (corpus_search, graph_query, entity_profile,
    timeseries_lookup, precedent_search), not through this function, because
    those tools need a live entitlement-checked connection, not a config.
    """
    tools = [tool(ResearchComplete), think_tool]

    # Track existing tool names to prevent conflicts
    existing_tool_names = {
        t.name if hasattr(t, "name") else t.get("name", "tool")
        for t in tools
    }
