...
async def test_request_carries_versions_and_closed_vocab(pipeline) -> None:
    client, sink = pipeline
    doc = await _doc("SW-2024-0001")
    stub = StubExtractor({})
    pipe = ExtractionPipeline(stub, AssertionWriter(client), sink)
    ...
    assert doc.body in request.prompt
...
async def test_extraction_produces_expected_events_with_dates(pipeline) -> None:
    ...
    pipe = ExtractionPipeline(stub, writer, sink)
    ...
    rows = await client.read_assertions(as_of=RECORDED)
    ...
    assert (maturity["valid_from"].year, maturity["valid_from"].month,
            maturity["valid_from"].day) == (2025, 5, 14)
...
async def test_offsets_must_resolve_to_real_text(pipeline) -> None:
    ...
    pipe = ExtractionPipeline(StubExtractor({"SW-2024-0005": [bad]}), writer, sink)
